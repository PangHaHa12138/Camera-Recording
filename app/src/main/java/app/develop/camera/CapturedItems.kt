...
import app.develop.camera.CamConfig.SettingValues
...
object CapturedItems {
    ...
    private fun collectMediaStoreItems(
        ...
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                ...
                resolver.query(volumeUri, columns, null, null, null)?.use {
                    dest.ensureCapacity(it.count)
    ...
    private fun collectSafItems(
        ...
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                ...
                resolver.query(childDocumentsUri, columns, null, null, null)?.use {
                    dest.ensureCapacity(it.count)
    ...
    fun savePreviousSafTrees(trees: List<Uri>, editor: SharedPreferences.Editor) {
        ...
        val str =
            trees.map { it.toString() }.toTypedArray().joinToString(separator = SAF_TREE_SEPARATOR)
        editor.putString(SettingValues.Key.PREVIOUS_SAF_TREES, str)
    }

    private fun migratePreviousUris(
        ctx: Context,
        camConfig: CamConfig,
        joinedUris: String,
